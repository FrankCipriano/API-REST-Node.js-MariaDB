    ...
    dao.get((err,rows)=>{
        if(err){
            const respuestas={
                ...
                descripcion: `Puede ser un error de Sintaxis SQL o que la base de datos esta fuera de servicio`,
                error:err
            }
...
//-AQUI ABAJO ESTAN LOS METODOS PARA EL VERBO POST Y EL UPDATE EN METODOS INDIVIDUALES
/*
Controlador.prototype.post=function(req,res,next){
    const afiche={
    ...
    dao.post(afiche,(err,rows)=>{
        if(err){
            const respuestas={
                title:`No fue posible registrar los datos de la pelicula con ID: ${afiche.MovieID}`,
                descripcion: `Puede ser un error de Sintaxis SQL o que la base de datos esta fuera de servicio`,
                error:err
            }
            return res.render(`404`,respuestas)
...
*/
/*
Controlador.prototype.put=function(req,res,next){
    const afiche={
        MovieID:    req.body.MovieID,
        Title:      req.body.Title,
        ReleaseMovie:req.body.ReleaseMovie,
    ...
    dao.put(afiche,(err,rows)=>{
        if(err){
            const respuestas={
                title:`No fue posible actualizar los datos de la pelicula con ID: ${afiche.MovieID}`,
                descripcion: `Puede ser un error de Sintaxis SQL o que la base de datos esta fuera de servicio`,
                error:err
            }
            return res.render(`404`,respuestas)
